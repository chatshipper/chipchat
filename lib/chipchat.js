    ...
    return (bot, payload, next) => {
        ...
        if (payload.event.startsWith('message')) {
            const other = payload.data.conversation.participants.find(
                ...
                && (bot._instances[payload.data.conversation.organization] || {}).id
                    // it is not the bot instance of this org
                    !== null !== p.user
                && (
            ...
            if (other) {
                debug(`dropping message ${payload.data.message.id} as ${other.role} ${other.name} is active`);
                return;
            }
        }
        next();
...
function ignoreDisabledBotsMiddleware() {
    return (bot, payload, next) => {
        ...
        if (!bot.preloadBots) {
            next('ignoreDisabledBotsMiddleware: enable preloadBots as well');
        }
        ...
        if (!orgId && payload.organization) {
            orgId = payload.organization;
        }
        if (orgId && !(bot.botconfig(orgId) || {}).enabled) {
            debug('ignoreDisabledBotsMiddleware: dropping message as bot is not enabled');
            return;
        }
        next();
    };
...
function botInstanceMiddleware() {
    return (bot, payload, next) => {
        ...
        if (!payload.event.startsWith('message') || !bot.auth) {
            return next();
        }
        const orgId = payload.data.conversation.organization;
        debug('find bots for', orgId, bot.auth.user);
        if (bot._instances[orgId]) {
            return next();
        }
        const options = { organization: orgId, bot: bot.auth.user, populate: 'bot' };
        return bot.botinstances.list(options).then((bots) => {
            if (bots.length === 0) {
                // org has no bot instance configured
                debug('botInstanceMiddleware: org has no bot instance');
                return next();
            }
            if (bots.length !== 1) {
                return next(`Single instance for bot ${bot.auth.user} not found (${bots.length})`);
            }
            const instance = bots[0];
            if (!instance.enabled || !instance.status === 'active') {
                return next('Instance not enabled and/or bot not active');
            }
            debug('preloaded bot', instance.id);
            const { status, roles, audience, tags, commands } = instance.bot;
            instance.bot = instance.bot.id;
            bot._instances[orgId] = {
                ...instance, status, roles, audience, tags, commands
            };
...
class ChipChat extends RestAPI(Server(EventEmitter2)) {
    ...
    /**
       ...
       @param {boolean} [options.preloadOrganizations=false] - prefetch organization record for
         each conversation
       @param {boolean} [options.preloadBots=false] - prefetch bot and bot instance records
    ...
    constructor(options = {}) {
        ...
        this.preloadBots = typeof opts.preloadBots === 'boolean' ? opts.preloadBots : false;
        ...
        this.ignoreDisabledBots = typeof opts.ignoreDisabledBots === 'boolean' ? opts.ignoreDisabledBots : false;
    ...
    /**
     * Get configuration settings of a bot instance of an organisation.
     * @param  {String}   organisation ID
     * @return {Object} instance config object
     */
    botconfig(organization) {
        if (!this.preloadBots) {
            debug('Enable the preloadBots option to be able to use this method.');
        }
        return this._instances[organization];
    }
