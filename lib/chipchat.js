/**
   ChipChat - Web1on1 SDK Main module
   @module ChipChat
   @author Web1on1
*/
...
function ignoreSelfMiddleware() {
    return (bot, payload, resolve, reject, next) => {
        if (!payload.event) {
            ...
            bot.emit('error', new Error('Invalid payload'));
...
function ignoreUnjoinedMiddleware() {
    return (bot, payload, resolve, reject, next) => {
        ...
        if (notActive && !(assign || command || mention)) {
            debug('ignoreUnjoined', bot.auth.user);
            if (resolve) resolve();
            return;
        }
        next();
...
class ChipChat extends RestAPI(Server(EventEmitter2)) {
    ...
    /**
     * Decode a JSON Web Token WITHOUT verifying its signature. Tokens whose
     * header algorithm is missing, `none`, or outside {@link JWT_ALGORITHMS}
     * are rejected, but no cryptographic verification is performed. The
     * returned claims are UNTRUSTED and must not drive an authorization
     * decision about a third party; use {@link ChipChat.verifyJwt} with a key,
     * or construct the bot with `options.jwksUri`/`options.jwtKey`, for that.
     * This decode path is the last-resort fallback: `jwksUri` (RS256/JWKS) takes
     * precedence, then `jwtKey`, then this decode.
     * @static
     * @param {string} token
     * @returns {object|null} the decoded payload, or null when the token is
     *   malformed or uses a disallowed algorithm
     */
    static decodeJwt(token) {
        const answer = jwt.decode(token, { complete: true });
        if (!answer || !answer.header || !JWT_ALGORITHMS.includes(answer.header.alg)) {
            return null;
        }
    ...
    /**
     * Verify a JSON Web Token's signature and return its claims, restricting
     * the accepted algorithms to an explicit allowlist that excludes `none`.
     * @static
     * @param {string} token
     * @param {string|Buffer} key - HS shared secret or RS/ES public key
     * @param {object} [options] - additional jsonwebtoken.verify options
     * @returns {object} the verified payload
     * @throws when the signature is invalid or the algorithm is not allowed
     */
    static verifyJwt(token, key, options = {}) {
        return jwt.verify(token, key, { algorithms: JWT_ALGORITHMS, ...options });
    }

    /**
     * Verify an access token against a JWKS endpoint (RS256) and populate
     * `this.auth` from the VERIFIED claims. The token header's `kid` selects
     * the signing key, which is fetched from `jwksUri` via jwks-rsa; the token
     * is then verified with RS256 only. On any failure an `error` event is
     * emitted and the returned promise rejects — there is no decode fallback.
     * @private
     * @param {string} token
     ...
     */
    async _verifyTokenViaJwks(token, jwksUri, refreshToken) {
        try {
            // eslint-disable-next-line global-require
            const jwksRsa = require('jwks-rsa');
            const client = jwksRsa({ jwksUri, cache: true, rateLimit: true });
            // jwt.verify reads the header (incl. kid) internally and asks the
            // provider for the key, so the token is never decoded unverified here
            const getKey = (header, callback) => {
                if (!header || !header.kid) {
                    callback(new Error('token is missing a kid header'));
                    return;
                }
            ...
            this.auth = { ...this.auth, ...decoded, user: decoded._id, token };
            if (/admin|bot/.test(scope) && refreshToken) {
                this.auth.refreshToken = refreshToken;
            }
            /* eslint-enable camelcase */
            this._authPending = null;
        } catch (e) {
            const err = new Error(`Invalid token: ${e.message}`, { type: 'constructor' });
            this.emit('error', err);
            throw err;
        }
    }
    ...
    /**
       ...
       @param {string} [options.token] - Web1on1 API Access Token
       @param {string} [options.refreshToken] - Web1on1 API Refresh Token
       ...
       @param {string} [options.secret] - Web1on1 webhook Secret
       @param {string} [options.jwksUri] - JWKS endpoint URL. When set, the access token is
         verified asynchronously with RS256 against the key selected by its `kid` header
         (see {@link ChipChat#_verifyTokenViaJwks}). This takes precedence over options.jwtKey
         and the decode fallback. Because the key lookup is async, `this.auth` is populated once
         {@link ChipChat#ready} resolves; message ingestion is gated on it automatically.
       @param {string|Buffer} [options.jwtKey] - Key used to verify the access token's
         signature (HS shared secret or RS/ES public key). When set (and options.jwksUri is not),
         the token is verified with {@link ChipChat.verifyJwt}; otherwise its claims are decoded
         without verification.
       @param {object} [options.jwtVerifyOptions] - Extra jsonwebtoken.verify options, used
         together with options.jwtKey.
       @param {string} [options.host] - Target Web1on1 API server, defaults to https://api.web1on1.chat
    ...
    constructor(options = {}) {
         ...
         */
        this.host = opts.host || process.env.APIHOST || 'https://api.web1on1.chat';
        /**
        ...
        this.on(['botinstance.*'], (event)=> {
            if (this.cache) {
                const botId = event?.data?.botinstance?.bot;
                const orgId = event?.organization;
                if (botId && orgId && this.auth.user === botId) {
                    const id = orgId + botId;
                    const now = new Date();
                    this.tempDisabledCache[id] = new Date(now.getTime() + 60 * 60 * 1000);
                    this.cache.del(id);
                }
            }
        });
        ...
        /**
         * Resolves once the access token (if any) has been authenticated. Only
         * meaningful for the asynchronous `jwksUri` path; it is an
         * already-resolved promise for the synchronous `jwtKey`/decode paths and
         * when no token is supplied. Consumers using `jwksUri` may `await
         * bot.ready` before relying on `bot.auth`; message ingestion is gated on
         * it automatically.
         * @name ChipChat#ready
         * @type {Promise}
         */
        this.ready = Promise.resolve();
        if (opts.token && opts.jwksUri) {
            this.ready = this._verifyTokenViaJwks(opts.token, opts.jwksUri, opts.refreshToken);
            this._authPending = this.ready;
            // error is also surfaced via the 'error' event; avoid an unhandled
            // rejection for consumers that do not await bot.ready
            this.ready.catch(() => {});
        } else if (opts.token) {
            let decoded;
            if (opts.jwtKey) {
                try {
                    decoded = ChipChat.verifyJwt(opts.token, opts.jwtKey, opts.jwtVerifyOptions);
                } catch (e) {
                    this.emit('error', new Error(`Invalid token: ${e.message}`, { type: 'constructor' }));
                    throw new Error(`Invalid token: ${e.message}`);
                }
            } else {
                decoded = ChipChat.decodeJwt(opts.token);
            }
            if (decoded) {
                /* eslint-disable camelcase */
                const { scope, grant_type } = decoded;
                ...
                if (grant_type === 'refresh_token') {
                    this.emit('error', new Error('Invalid token grant_type refresh_token', { type: 'constructor' }));
                    throw new Error('Invalid token grant_type refresh_token');
                }
                /**
    ...
    /**
     * Listen for [ChatShipper webhooks](https://developers.web1on1.chat/docs/pg-webhooks/),
     ...
     * event](https://developers.web1on1.chat/docs/pg-webhooks/#section-8-2-webhook-events).
     * @param {object} [conditionals] - Matching conditions key-value pairs
     * @param {function(Object)} handler - The handler to call. can be a registered callback name
     */
    on(events, conditionals, callback) {
        ...
        if (typeof callback === 'string' && this._registeredCallbacks[callback]) {
            callback = this._registeredCallbacks[callback];
        }
        if (typeof conditionals === 'string' && this._registeredCallbacks[conditionals]) {
            callback = this._registeredCallbacks[conditionals];
     ...
     * @param  {RegExp|array}   regexp       RegExp to be executed with `exec`.
     * @param {object} [conditionals] - Matching conditions key-value pairs
     * @param {function(Object)} handler - The handler to call. can be a registered callback name
     */
    onText(triggers, conditionals, callbackProto) {
        ...
        if (!callbackProto && typeof conditionals === 'string' && this._registeredCallbacks[conditionals]) {
            callback = (m, c) => {
                debug('matched cond onText, calling callback name');
                this._registeredCallbacks[conditionals](m, c);
            };
        } else if (callbackProto) {
            callback = (m, c) => {
                if (match(c, m, conditionals)) {
                    debug('matched cond onText');
                    if (typeof callbackProto === 'string' && this._registeredCallbacks[callbackProto]) {
                        this._registeredCallbacks[callbackProto](m, c);
                    } else {
                        callbackProto(m, c);
                    }
                }
    ...
    _sendErrorEvent(error, opts) {
        ...
        if (opts.async) {
            this.emitAsync('error', new Error(error.message, { type: error.type }));
        } else {
            this.emit('error', new Error(error.message, { type: error.type }));
        }
    }
    ...
    ingest(payload, signature, options = {}) {
        if (this._authPending) {
            // jwksUri path: defer processing until the token is verified so
            // this.auth.user is reliably set; drop the message if auth fails
            return this._authPending.then(
                () => this.ingest(payload, signature, { ...options, async: true }),
                () => undefined
            );
        }
        ...
        if (payload.event.startsWith('message')) {
            ...
            if (options.async) {
                ...
                return new Promise((resolve, reject) => {
                    ...
                    debug('ingest: promise was resolved');
                });
            }
    ...
    _handleEvent(type, event, resolve, reject) {
        ...
        this.middleware.receive.run([this, event, resolve, reject], (err) => {
            if (err) {
                ...
                this.emit('error', new Error(`middleware receive processing error:  ${error.message || error.toString()}`, { type: 'middleware' }));
    ...
    _handleCatch(message, type = 'Error', returnVal) {
        return (error) => {
            ...
            this.emit('error', new Error(`${message}${errorMessage}`, { type }));
            return returnVal;
        };
    }
    ...
    _handleMessage(type, message, resolve, reject) {
        ...
        if (type !== 'channel.notify'
            ...
            this._textRegexpCallbacks.some(({ regexp, callback }) => {
                ...
                ctx.captured = true;
                if (resolve) {
                    allResolves.push(new Promise((r) => {
                        (async () => {
                            await callback(message, ctx);
                            r();
                        })();
                    }));
                } else {
    ...
    conversation(conversation, cb) {
        if (!conversation) { // || this._conversations[conversation]) {
            this.emit('error', new Error(`missing conversation ${conversation}`, { type: 'conversation' }));
    ...
    _getActionsObject(convId) {
        ...
        if (!convId || !this._conversations[convId]) {
            this.emit('error', new Error(`missing conv ${convId}`, { type: 'conversation' }));
            return {};
        }
        ...
        return {
            ...
            ask: (q, cb) => {
                const afterSendingMessage = (callback) => (e, msg) => {
                    if (e) { return this.emit('error', new Error(e)); }
                ...
                return new Promise((resolve, reject) => {
                    this.send(convId, q)
                        ...
                        .catch((error) => { this.emit('error', new Error(error.message)); reject(error); });
                });
            }
        };
    }
