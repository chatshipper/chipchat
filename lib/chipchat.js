...
class ChipChat extends RestAPI(Server(EventEmitter2)) {
    ...
    /**
       ...
       @param {string} [options.webhook] - Webhook path segment, defaults to '/'
    ...
    constructor(options) {
        ...
        this.on('conversation.update', (event) => {
            if (this.conversations[event.data.conversation.id]) {
                this.conversations[event.data.conversation.id] = event.data.conversation;
            }
        });
    ...
    _handleMessage(type, message) {
        ...
        if (type !== 'channel.notify' && message.type === 'chat' && message.text
            ...
            this.sendMessage(
                ...
                (err) => {
                    if (err) {
                        ...
                        this.emit('error', { message: `${text} error: ${err.toString()}` });
                    }
                }
            );
        };
        ...
        return Object.assign({}, this.conversations[convId] || {}, {
            ...
            set: (key, val) => {
                ...
                if (matches && conversation[matches[1]]) {
                    ...
                    ((this.conversations[convId] || {}).meta || {})[key] = val;
                }
                command(`/set ${key} ${val}`);
            },
            get: (key) => {
