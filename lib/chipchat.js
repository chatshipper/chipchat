...
function botInstanceMiddleware() {
    return (bot, payload, resolve, reject, next) => {
        ...
        if (bot._instances[orgId]) {
            debug(`botInstanceMiddleware: returning instance of org ${orgId} from cache`);
        ...
        return bot.botinstances.list(options).then((bots) => {
            if (bots.length === 0) {
                ...
                debug(`botInstanceMiddleware: org ${orgId} has no bot instance, providing empty default`);
                bot._instances[orgId] = defaultInstance;
                payload.instance = bot._instances[orgId];
                return next();
            }
            if (bots.length !== 1) {
                if (reject) reject(`Single instance for bot ${bot.auth.user} not found (${bots.length})`);
            ...
            const { meta = {}, status, roles, audience, tags, commands } = instance.bot;
            ...
            bot._instances[orgId] = {
                ...instance, meta: mergedMeta, status, roles, audience, tags, commands
            };
            payload.instance = bot._instances[orgId];
...
class ChipChat extends RestAPI(Server(EventEmitter2)) {
    ...
    /**
       ...
       @param {boolean} [options.ignoreDisabledBots=true] - ignore bots that are disabled in store.
    ...
    constructor(options = {}) {
        ...
        this.ignoreDisabledBots = typeof opts.ignoreDisabledBots === 'boolean' ? opts.ignoreDisabledBots : true;
