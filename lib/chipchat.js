...
class ChipChat extends RestAPI(Server(EventEmitter2)) {
    ...
    constructor(options = {}) {
        ...
        this.on('botinstance.update', async (event) => {
            const org = event.data.botinstance.organization;
            debug(`botinstance.update: clearing instance cache of org ${org} and all suborgs`);
            ...
            debug(`botinstance.update: found ${instances.length} instances to check`, instances);
            const organizations = instances.filter(i => i.orgPath.includes(org) || org === '59c04c3c7d47307af611b1ec')
                .map(i => i.organization);
            debug(`botinstance.update: found ${organizations.length} instances to clear`, organizations);
            organizations.forEach((organization) => {
                if (this._instances[organization]) {
                    debug(`botinstance.update: clearing instance cache of org ${organization}`);
