...
class ChipChat extends RestAPI(Server(EventEmitter2)) {
    ...
    _handleEvent(type, event) {
        ...
        this.middleware.receive.run([this, event], (err) => {
            if (err) {
                debug('receive middleware returned error', err);
                this.emit('error', { type: 'middleware', message: `middleware receive processing error:  ${((err.body || {}).error || {}).message}` });
                return;
            }
            debug('emit activity');
    ...
    _handleMessage(type, message) {
        ...
        if (type !== 'channel.notify'
            ...
            if (questionAsked) {
                debug('questionAsked', `_asked${this.auth.user}`, this._replyListeners.length);
