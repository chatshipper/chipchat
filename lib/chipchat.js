...
class ChipChat extends RestAPI(Server(EventEmitter2)) {
    ...
    conversation(conversation, cb) {
        if (!conversation) { // || this._conversations[conversation]) {
            this.emit('error', { message: `missing conv ${conversation}` });
            const err = new Error(`missing conv ${conversation}`);
    ...
    _getActionsObject(convId) {
        ...
        const command = (text) => {
            return new Promise((resolve, reject) => {
                this.send(
                    ...
                    (err) => {
                        if (err) {
                            this.emitAsync('error', { message: `${text} error: ${err.toString()}` }).then(reject);
                        }
                        resolve();
                    }
                );
        ...
        return Object.assign({}, this._conversations[convId] || {}, {
            ...
            say: (...args) => {
                return new Promise((resolve, reject) => {
                    this.send.apply(this, [convId, ...args, (err) => {
                        if (err) {
                            reject(err);
                        } else {
                            resolve();
            ...
            ask: (q, cb) => {
                return new Promise((resolve, reject) => {
                    this.send(convId, q, (e, msg) => {
                        if (e) { this.emitAsync('error', e).then(reject); }
