...
function botInstanceMiddleware() {
    return (bot, payload, next) => {
        ...
        if (bot._instances[orgId]) {
            debug(`botInstanceMiddleware: returning org ${orgId} instance from cache`);
...
class ChipChat extends RestAPI(Server(EventEmitter2)) {
     ...
     */
    pringest(payload, signature) {
        return new Promise((resolve) => {
            this.ingest(payload, signature, resolve);
        });
    }
     ...
     * @param  {Object} payload - the full webhook payload
     * @param  {String} [signature] - x-hub-signature http header, a HMAC to verify the payload
     * @param  {Function} resolve - (optional) use emitAsync and resolve when done
     */
    ingest(payload, signature, resolve) {
        ...
        if (payload.event.startsWith('message')) {
            ...
            this._handleEvent(payload.event, payload, resolve);
    ...
    /**
       @private
     */
    _handleEvent(type, event, resolve) {
        ...
        this.middleware.receive.run([this, event], (err) => {
            ...
            if (isMessage && event.data && event.data.message) {
                this._handleMessage(type, event.data.message, resolve);
            } else {
                debug('emit', type);
    ...
    /**
       @private
     */
    _handleMessage(type, message, resolve) {
        ...
        if (!hasReplied) {
            ...
            if (resolve) {
                this.emitAsync('message', message, ctx).then(resolve);
            } else {
