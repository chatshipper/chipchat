    ...
    return (bot, payload, resolve, reject, next) => {
        ...
        return bot.bots.get(bot.auth.user).then((botinfo) => {
            const defaultInstance = {
                meta: {},
                enabled: botinfo.meta.audience === 'owner',
                status: botinfo.meta.audience === 'owner' ? 'active' : 'not installed'
            };
            const orgPathRoot = (payload.data.message && payload.data.message.orgPath)
                || payload.organization;
            const paths = orgPathRoot.split('#').reduce((arr, item) => { arr.push([...arr.slice(-1) || [], item].join('#')); return arr; }, []);
            const options = { orgPath: `[${paths.join(',')}]`, sort: 'orgPath', bot: botinfo.id };
