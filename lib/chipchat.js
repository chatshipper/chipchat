...
class ChipChat extends RestAPI(Server(EventEmitter2)) {
    ...
    constructor(options = {}) {
        ...
        this.on('botinstance.update', this._clearInstanceCache);
        this.on('botinstance.delete', this._clearInstanceCache);

        this._initApi(opts);
    }

    async _clearInstanceCache(event) {
        const org = event.data.botinstance.organization;
        debug(`_clearInstanceCache: clearing instance cache of org ${org} and all suborgs`);
        const bot = event.data.botinstance.bot;
        const instances = await this.botinstances.list({ bot }).catch(this._handleCatch('error clearing cache', 'botinstance', []));
        debug(`_clearInstanceCache: found ${instances.length} instances to check`, instances);
        const organizations = instances.filter(i => i.orgPath.includes(org) || org === '59c04c3c7d47307af611b1ec')
            .map(i => i.organization);
        debug(`_clearInstanceCache: found ${organizations.length} instances to clear`, organizations);
        organizations.forEach((organization) => {
            if (this._instances[organization]) {
                debug(`_clearInstanceCache: clearing instance cache of org ${organization}`);
                delete this._instances[organization];
            }
        });
    }
    ...
    ingest(payload, signature, options = {}) {
        ...
        // invalid payload
            ...
            this._sendErrorEvent({ type: 'unknown', message: `unknown event: ${payload.event}` });
        }
    }
    ...
    /**
       @private
     */
    async _updateInstanceConfiguration(payload) {
        if (!this.preloadBots) {
            return;
        }
        if (!this.auth) {
            this.emit('error', { type: 'botinstance', message: 'Need to have authentication...' });
            return;
        }
        const orgId = payload.organization;
        debug(`_updateInstanceConfiguration: ${orgId}: find bots for user ${this.auth.user}...`);
        if (this._instances[orgId]) {
            debug(`_updateInstanceConfiguration: ${orgId}: returning instance from cache`, this._instances[orgId].meta);
            return;
        }
        debug(`_updateInstanceConfiguration: ${orgId}: getting botinfo ${this.auth.user}`);
        const botinfo = await this.bots.get(this.auth.user).catch(this._handleCatch('Could not load bot info', 'bot'));
        if (!botinfo) return;
        ...
        const orgs = (payload.orgPath || payload.organization).split('#');
        ...
        const instances = await this.botinstances.list(options).catch(this._handleCatch('Could not load instances', 'botinstance'));
        if (!instances) return;
        instances.forEach((instance) => {
            // replace allInstances element of this instance
        ...
        if (!lastInstance.enabled) {
            ...
            debug(`_updateInstanceConfiguration: ${orgId}: bot instance ${lastInstance.id} is not enabled, providing empty default`, defaultInstance);
            this._instances[orgId] = defaultInstance;
            return;
        }
        const mergedMeta = {};
        ...
        debug(`_updateInstanceConfiguration: ${orgId}: updating instance config cache to:`, mergedMeta);
        this._instances[orgId] = { ...instances[0], bot: botinfo.id, meta: mergedMeta };
    }
    ...
    _handleMessage(type, message, resolve, reject) {
        ...
        if (!hasReplied) {
            ...
            this._updateInstanceConfiguration(message, ctx).then(() => {
                const instance = this._instances[message.organization] || { meta: {}, enabled: false, status: 'not found' };
