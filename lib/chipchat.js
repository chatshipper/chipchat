...
class ChipChat extends RestAPI(Server(EventEmitter2)) {
    ...
    ingest(payload, signature) {
        ...
        if (payload.event.startsWith('message')) {
            ...
            this._conversations[payload.data.conversation.id] = Object.assign(
                {},
                this._conversations[payload.data.conversation.id] || {},
    ...
    _getActionsObject(convId) {
        ...
        const set = (key, val) => {
            ...
            if (matches && conversation[matches[1]]) {
                ...
                if (!this._conversations[convId]) this._conversations[convId] = {};
                if (!this._conversations[convId].meta) this._conversations[convId].meta = {};
                this._conversations[convId].meta[key] = val;
            }
            debug('_conversations[convId]', this._conversations[convId]);
        ...
        return Object.assign({}, this._conversations[convId] || {}, {
            ...
            ask: (q, cb) => {
                const afterSendingMessage = callback => (e, msg) => {
                    ...
                    set(`_asked${this.auth.user}`, msg.id);
                    const id = this.onReply(convId, msg.id, (m) => {
                        ...
                        if (typeof callback === 'string') {
                            this._registeredCallbacks[callback](m, c);
                        } else {
                            callback(m, c);
                        }
                    });
                ...
                return new Promise((resolve, reject) => {
                    this.send(convId, q)
                        .then(afterSendingMessage.bind(this, resolve))
