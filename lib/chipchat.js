...
class ChipChat extends RestAPI(Server(EventEmitter2)) {
    ...
    on(events, conditionals, callback) {
        ...
        if (!callback && conditionals && typeof conditionals === 'function') {
    ...
    sendErrorEvent(error) {
        ...
        if (this.listeners('error').length < 2) {
            console.warn('you need to register at least the error event to catch errors', error.message);
        }
    }
    ...
    ingest(payload, signature) {
        // do we want to wait for this to finish (use promise)?
        // botmanager uses a promise wrapper and resolves on every on event
        // because they are all unique it the case of bot manager
        if (signature) {
            const calculated = getSignature(JSON.stringify(payload), this.secret);
            if (calculated !== signature) {
                return this.sendErrorEvent({ type: 'ingest', message: 'Message integrity check failed' });
            }
        }
        if (!payload || !payload.event) {
            return this.sendErrorEvent({ type: 'ingest', message: 'Invalid payload, missing event' });
        }
        ...
        if (payload.event.startsWith('message')) {
            if (!payload.data || !payload.data.conversation || !payload.data.conversation.id) {
                this.sendErrorEvent({ type: 'ingest', message: 'Invalid payload, missing conversation' });
            }
            ...
            if (!this._organizations[payload.data.conversation.organization]
                ...
                const org = this.organizations.get(payload.data.conversation.organization, { populate: 'categories.forms' }).catch((err) => {
    ...
    /**
       @private
     */
    async _handleMessage(type, message) {
    ...
    _getActionsObject(convId) {
        ...
        return Object.assign({}, this._conversations[convId] || {}, {
            ...
            ask: (q, cb) => {
                const afterSendingMessage = callback => async (e, msg) => {
                    if (e) { return this.emit('error', e); }
                    await command(`/set _asked${this.auth.user} ${msg.id}`);
                    const id = await this.onReply(convId, msg.id, (m) => {
                        debug('ask.answered', m.text);
                        this.removeReplyListener(id);
                        const c = this._getActionsObject(convId);
                        setTimeout(async () => {
                            await command(`/set _asked${this.auth.user}`);
                            if (typeof callback === 'string') {
                                await this._registeredCallbacks[callback](m, c);
                            } else {
                                await callback(m, c);
                            }
                        }, 0);
                ...
                if (cb) {
                    this.send(convId, q, afterSendingMessage(cb));
                    return null;
                }
                return new Promise((resolve, reject) => {
                    try {
                        this.send(convId, q, afterSendingMessage((m, c) => {
                            resolve([m, c]);
                        }));
                    } catch (e) {
                        reject(e);
