...
function botInstanceMiddleware() {
    return (bot, payload, resolve, reject, next) => {
        ...
        const defaultInstance = {
            meta: {},
            enabled: bot.meta.audience === 'owner',
            status: bot.meta.audience === 'owner' ? 'active' : 'not installed'
        };
        ...
        const options = { orgPath: `[${paths.join(',')}]`, sort: 'orgPath', bot: bot.auth.user, populate: 'bot' };
        return bot.botinstances.list(options).then((bots) => {
            if (bots.length === 0) {
                // org has no bot instance configured
                debug(`botInstanceMiddleware: org ${orgId} has no bot instance, providing empty default`, defaultInstance, bot.meta);
            ...
            bot._instances[orgId] = { ...bots[0], bot: bots[0].bot.id, meta: mergedMeta };
            payload.instance = bot._instances[orgId];
