        ...
        return new Promise(async (resolve, reject) => {
            if (signature) {
                ...
                if (calculated !== signature) {
                    ...
                    return reject(new Error('Message integrity check failed'));
                }
            }
            if (!payload || !payload.event) {
                this.emit('error', { type: 'ingest', message: 'Invalid payload, missing event' });
                return reject(new Error('Invalid payload, missing event'));
            }
            ...
            if (payload.event.startsWith('message')) {
                if (!payload.data || !payload.data.conversation || !payload.data.conversation.id) {
                    ...
                    return reject(new Error('Invalid payload, missing conversation'));
                }
                ...
                if (!this._organizations[payload.data.conversation.organization]
                    ...
                    const org = await this.organizations.get(payload.data.conversation.organization, { populate: 'categories.forms' }).catch((err) => {
                        ...
                        this.emit('error', { message: errorMessge });
                        return reject(new Error(errorMessge));
                    });
                    debug('cachedorg', org.id);
                    this._organizations[org.id] = org;
                    //payload.data.conversation.organization = org;
                }
                return this._handleEvent(payload.event, payload).then(resolve).catch(reject);
            }
            ...
            return reject(new Error('unknown event'));
        });
    }
