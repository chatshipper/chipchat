...
// queue used for incoming event delay
const incomingQueue = [];
/**
 * Middleware that automatically adds delay to each incoming event
 * spreading them out so a minimmum time distance of 1 second is
 * between them. This gives bots time to store and get conversation
 * state.
 *
 * @api private
 */

function delayQueueResolver(timeout = 1000) {
    return setInterval(() => {
        const next = incomingQueue.pop();
        if (next) next();
    }, timeout);
}

function autoQueueMiddleware() {
    return (bot, payload, resolve, reject, next) => {
        incomingQueue.push(next);
    };
}
...
function ignoreOtherParticipantsMiddleware() {
    return (bot, payload, resolve, reject, next) => {
        ...
        if (payload.event.startsWith('message')) {
            ...
            if (other) {
                debug(`dropping message ${payload.data.message.id} as ${other.role} ${other.name}(${other.id}) is active`);
                if (resolve) resolve();
                return;
            }
        }
...
/**
 * Middleware that ignores messages when botinstance is not enabled
 *
 * @api private
 */
function ignoreDisabledBotsMiddleware() {
    return (bot, payload, resolve, reject, next) => {
        if (!payload.event) {
            console.log('invalidpayload', JSON.stringify(payload));
            bot.emit('error', { code: 'Invalid payload' });
        ...
        if (payload.instance && (!payload.instance.enabled || !payload.instance.status === 'active')) {
            debug('ignoreDisabledBotsMiddleware: dropping message as bot is not enabled or inactive');
            if (resolve) resolve();
            return;
        }
        next();
...
function botInstanceMiddleware() {
    return (bot, payload, resolve, reject, next) => {
        if (!payload.event) {
            //bot.emit('error', { code: 'Invalid payload' });
            if (reject) reject('Invalid payload');
        ...
        if (!bot.auth) {
            if (reject) reject('Bot needs to be authenticated');
            return next('Bot needs to be authenticated');
        }
        const orgId = payload.organization;
        debug(`find bots in org ${orgId} for user ${bot.auth.user}`);
        if (bot._instances[orgId]) {
            debug(`botInstanceMiddleware: returning instance of org ${orgId} from cache`, bot._instances[orgId].meta);
            payload.instance = bot._instances[orgId];
            return next();
        }
        return bot.bots.get(bot.auth.user).then((botinfo) => {
            const defaultInstance = {
                meta: botinfo.audience === 'owner' ? botinfo.meta : {},
                enabled: botinfo.audience === 'owner',
                status: botinfo.audience === 'owner' ? 'active' : 'not installed'
            };
            const orgPathRoot = (payload.data.message && payload.data.message.orgPath)
                || payload.organization;
            const options = { organization: `[${orgPathRoot.replace(/#/g, ',')}]`, sort: 'orgPath', bot: botinfo.id };
            return bot.botinstances.list(options).then((bots) => {
                if (bots.length === 0) {
                    // org has no bot instance configured
                    debug(`botInstanceMiddleware: org ${orgId} has no bot instance, providing empty default`, defaultInstance);
                    bot._instances[orgId] = defaultInstance;
                    payload.instance = bot._instances[orgId];
                    return next();
                }
                const mergedMeta = {};
                debug(`botInstanceMiddleware: merging ${bots.length} configs`, bots.map(b => ({ id: b.id, ...b.meta })));
                bots.forEach((i) => {
                ...
                debug('preloading bot instance meta:', mergedMeta);
                bot._instances[orgId] = { ...bots[0], bot: botinfo.id, meta: mergedMeta };
                payload.instance = bot._instances[orgId];
                // clear cache after 15 minutes
                setTimeout(() => {
                    delete bot._instances[orgId];
                }, 15 * 60 * 1000);
                return next();
            }).catch((err) => {
                debug('error loading bots', err.toString());
                const errorMessge = `Could not load organization ${payload.data.conversation.organization}: ${err.toString()}`;
                if (reject) reject(errorMessge);
                return next(errorMessge);
            });
        }).catch((err) => {
            debug('error loading bot info', err.toString());
            const errorMessge = `Could not load bot ${bot.auth.user}: ${err.toString()}`;
            if (reject) reject(errorMessge);
            return next(errorMessge);
        });
    };
...
class ChipChat extends RestAPI(Server(EventEmitter2)) {
    ...
    constructor(options = {}) {
        ...
        if (opts.email) {
            ...
            const user = (opts.email.match(/bot\+(\S+)@chatshipper.com/i) || [])[1];
            if (user) { // bot id
                this.auth.user = user;
            }
        }
        ...
        this.autoQueueIncoming = typeof opts.autoQueueIncoming === 'boolean' ? opts.autoQueueIncoming : false;

        this.middleware = {
            receive: mware(),
            send: mware()
        };
        if (this.autoQueueIncoming) {
            this.delayResolverInverval = delayQueueResolver();
            this.middleware.receive.use(autoQueueMiddleware());
        }
        if (this.ignoreSelf) {
        ...
        if (this.preloadBots) {
            this.middleware.receive.use(botInstanceMiddleware());
            this.middleware.receive.use(ignoreDisabledBotsMiddleware());
        }
        ...
        this.on('instance.update', (event) => {
            debug(`clearing instance cache of org ${event.data.organization.id}`);
            delete this._instances[event.data.organization.id];
        });
    ...
    _handleMessage(type, message, resolve, reject) {
        ...
        if (!hasReplied) {
            ...
            if (resolve) Promise.all(allResolves).then(resolve).catch(reject);
        }
        if (resolve) resolve();
    }
    ...
    _getActionsObject(convId) {
        ...
        return {
            ...
            instance: this._instances[this._conversations[convId].organization],
