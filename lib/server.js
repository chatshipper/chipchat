        ...
        */
        ...
        napp.get(npath, (req, res) => {
            if (req.query.type === 'subscribe' && req.query.challenge) {
                ...
                this.emit('error', { message: `Invalid GET request to: '${req.hostname + req.url}' from ip: '${req.ip}' by useragent: '${req.headers['user-agent']}'` });
                res.status(403).end();
            }
        });
            ...
            try {
                if (opts.async) {
                    ...
                    this.ingest(req.body, null, opts).then((done) => {
                        ...
                        this.emit('error', { message: `Async ingest error on incoming request: ${e.toString()}` });
