...
const Server = (Webhooks) => class extends Webhooks {
    ...
    start(port) {
        ...
        this.router({ pathPrefix: this.webhook, app: this.app, async: true, cors: true });
        ...
        this.server.listen(this.app.get('port'), (err) => {
            if (err) {
                this.emit('error', new Error(`Server start error: ${err.toString()}`));
            }
    ...
    router(pathPrefix, app) {
        ...
        */
        const verifyRequest = (req, res, buf, encoding) => {
            ...
            if (expected !== calculated) {
                ...
                this.emit('error', new Error('Invalid signature on incoming request'));
            } else {
        ...
        */
        ...
        /**
           Set up Web1on1 Webhook GET Verification Endpoint
         */
        napp.get(npath, (req, res) => {
            if (req.query.type === 'subscribe' && req.query.challenge) {
                ...
                this.emit('error', new Error(`Invalid GET request to: '${req.hostname + req.url}' from ip: '${req.ip}' by useragent: '${req.headers['user-agent']}'`));
                res.status(403).end();
            }
        });
        ...
        */
        const abortOnValidationError = (err, req, res, next) => {
            if (err) {
                debug(`** Invalid X-HUB signature on incoming request! ${err.toString()}`);
                this.emit('error', new Error(`Invalid X-HUB signature on incoming request: ${err.toString()}`));
        ...
        */
        ...
        /**
           Setups the prefix handler for Web1on1 webhook calls
        ...
        */
            ...
            try {
                if (opts.async) {
                    ...
                    this.ingest(req.body, null, opts).then((done) => {
                        ...
                        debug('router: async ingest got error, sending 400 bad request', e);
                        this.emit('error', new Error(`Async ingest error on incoming request: ${e.toString()}`));
                        res.status(400).json({ error: e.toString() }).end();
                    });
                } else {
                    res.status(200).end();
        ...
        if (opts.cors) {
            napp.options(npath, cors()); // enable pre-flight request for POST request
            napp.post(npath, cors(), nhandler);
        } else {
