...
const Server = (Webhooks) => class extends Webhooks {
     ...
     */
    router(pathPrefix, app, options = {}) {
        const npath = pathPrefix && pathPrefix.charAt(0) !== '/' ? `/${pathPrefix}` : (pathPrefix || '/');
        const napp = app || Router();
        const opts = (typeof options === 'function') ? { handler: options } : options;
        ...
        */
        const nhandler = opts.handler || ((req, res) => {
            res.status(200).end();
            try {
