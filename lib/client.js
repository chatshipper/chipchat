...
const RestAPI = API => class extends API {
     ...
     */
    _initApi() {
        const callbackOrPromise = (isCallback, promise) => { return isCallback ? null : promise; };
        RESOURCES.forEach((path) => {
            this[path] = {
                ...
                get: (id, options, cb) => {
                    ...
                    return callbackOrPromise(cb, this._request('GET', `${path}/${id}`, options, cb));
                },
                create: (body, cb) => callbackOrPromise(cb, this._request('POST', path, body, cb)),
                update: (id, body, cb) => callbackOrPromise(cb, this._request('PATCH', `${path}/${id}`, body, cb)),
                delete: (id, cb) => callbackOrPromise(cb, this._request('DELETE', `${path}/${id}?force: true`, null, cb))
            };
        });
        // backwards-compat
    ...
    /**
       ...
       @private
    */
    async _request(method, path, json, cb) {
        ...
        const tryRefreshToken = async (err) => {
            debug('error', err.toString(), this.auth.refreshToken);
            // refresh token error
            if (this.auth.refreshToken && /jwt expired/i.test(err.toString())) {
                debug('refresh token');
                try {
                    const tokens = await this._refreshToken();
                    debug('refreshed, recall request', tokens);
                    this.token = tokens.token;
                    this.auth.refreshToken = tokens.refreshToken;
                    this.emit('token', tokens.token);
                    const answer = await this._request(method, path, json, cb);
                    return { body: answer };
                } catch (errRefresh) {
                    debug('refresh token error, throw', errRefresh.toString());
                    return { body: { error: errRefresh } };
                }
            }
            // otherwise return error
            return { body: { error: err } };
        };
        const response = await request(options).catch(tryRefreshToken);
        const body = response.body;
        // errors can come in the form of body.error
        if (body && body.error) {
            if (cb && typeof cb === 'function') {
                debug('request error, calling cb');
                setTimeout(cb.bind(cb, body.error, null), 0); //next tick
            }
        ...
        // The callback argument is merely a legacy interface, also called
        if (cb && typeof cb === 'function') {
    ...
    _refreshToken(cb) {
        ...
        return this._request('POST', 'auth/token', payload, cb);
    }
    ...
    */
    send(conversation, payload, extra, cb) {
        ...
        // middleware runner that handles both cb and reject
        const runMiddleware = (resolve, reject) => {
            ...
            this.middleware.send.run([this, payload], (err) => {
                ...
                if (!handleError(err, reject)) {
                    debug(`postdata with ${cb ? 'callback' : 'promise'}`, JSON.stringify(payload));
                    this._request('POST', `conversations/${conversation}/messages`, payload, cb)
                        .then(cb || resolve).catch(handleError.bind(handleError, reject));
                }
            });
        };

        // if callback is not used, we expect a promise
        if (!cb) {
            return new Promise((resolve, reject) => {
                runMiddleware(resolve, reject);
            });
        }
        runMiddleware();
    }
