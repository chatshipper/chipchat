...
const RESOURCES = [
    ...
    'partners', 'tests', 'testresults', 'links'
];
    ...
    const extFetch = fetch.extend({
        hooks: {
            ...
            beforeRequest: (opts.beforeRequest || []).concat([
                async (options) => {
                    const tokens = await instance._getTokens.call(instance);
                    if (tokens.token) {
                        options.headers = {
                            ...options.headers,
            ...
            afterResponse: (opts.afterResponse || []).concat([
                async (response, retryWithMergedOptions) => {
                    if (response.statusCode === 401 && instance.auth.refreshToken
                        ...
                        debug(`token ${instance.auth.token.slice(-10)} is not valid anymore`);
                        const options = {
                            headers: {
                        ...
                        if (instance.getTokens && typeof instance.getTokens === 'function') {
                            // do I have store tokens that might work, lets try that first
                            debug('afterResponse: getting tokens from instance');
                            const storeTokens = await instance.getTokens.call(instance);
                        ...
                        try {
                            const tokens = await extFetch(instance._getOptions('POST', 'auth/token', payload));
                            debug('new tokens', tokens.body);
                            const newTokens = tokens.body;
                            await instance._setTokens(newTokens);
                            debug('new tokens stored');
                            instance.emit('token', newTokens.token);
                            return retryWithMergedOptions(options);
                        } catch (e) {
                            debug('error storing new tokens', e);
                            if (instance.resetTokenCache) {
                                instance.resetTokenCache(instance.auth.user);
                            }
                            throw new Error('Could not get tokens with refreshToken');
                        }
                    }
...
const RestAPI = (API) => class extends API {
    ...
    _initApi(opts) {
        ...
        this.cache = new Cache(3600 * 1000);
        this.tempDisabledCache = {};
    ...
    */
    async _getTokens() {
        ...
        if (this.auth && this.auth.token) {
            // return cache
            debug('_getTokens: return auth.token', this.auth.token.slice(-10));
            return this.auth;
        }
        if (typeof this.getTokens === 'function') {
            debug('using mixin getTokens');
            const tokens = await this.getTokens.call(this);
    ...
    */
    _getOptions(method, path, json) {
        ...
        // String path is relative, urls passed as URL object are absolute
        const options = {
            method,
            responseType: 'json',
            url: typeof path === 'string' ? `v2/${path}${querystring}` : path.toString()
        };
        if (typeof path === 'string') {
            options.prefixUrl = this.host;
        }
    ...
    */
    _request(method, path, json, cb) {
        ...
        const req = this.fetch(options)
            ...
            .catch((err) => {
                throw new Error('failed to fetch non-paginated request');
            });
        if (!cb) return req;
        return true;
    }
    ...
    send(conversation, payload, extra, cb) {
        ...
        const handleError = (callback, reject, err) => {
            if (err) {
                ...
                this.emit('error', new Error(`middleware send processing error: ${error.message || error}`, { type: 'middleware' }));
                if (callback) {
                    callback(error, null);
                } else if (reject) {
                    reject(error);
        ...
        const debouncer = (conversation, payload, callback, resolve, reject, delay) => {
            if (!this.requests[conversation]) {
                this.requests[conversation] = [];
            }
            this.requests[conversation].push({
                payload,
                callback,
                resolve,
                reject
            });
            clearTimeout(this.inDebounce[conversation])
            this.inDebounce[conversation] = setTimeout(async () => {
                if (this.requests[conversation]) {
                    const payloads = []
                    this.requests[conversation].forEach(r=>{
                        if (Array.isArray(r.payload)) payloads.push(...r.payload)
                        else payloads.push(r.payload)
                    })
                    const requests = [...this.requests[conversation]]
                    delete this.requests[conversation];
                    delete this.inDebounce[conversation];
                    await this._request(
                        'POST',
                        ...
                        .then((res)=>{
                            requests.forEach(r=>{
                                r.callback? r.callback(null, res): r.resolve(res)
                            })
                        })
                        .catch((e)=>{
                            requests.forEach(r=>{
                                handleError.bind(this, r.callback, reject)(e)
                            })
                        })
                } else {
        ...
        const runMiddleware = (callback, resolve, reject) => {
            ...
            this.middleware.send.run([this, payload], (err) => {
                ...
                if (!handleError(callback, reject, err)) {
                    ...
                    if (BATCHMESSAGES) {
                        debouncer.call(this, conversation, payload, callback, resolve, reject, 300)
                    } else {
