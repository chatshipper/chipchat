...
const RestAPI = API => class extends API {
    ...
    _request(method, path, json, cb) {
        ...
        return request({
            ...
            headers: {
                ...
                'Content-Length': method === 'POST' || method === 'PATCH' ? json.length : null
            },
    ...
    */
    sendMessage(conversation, payload, extra, cb) {
        ...
        this.middleware.send.run([this, data], (err) => {
            ...
            this._request('POST', `conversations/${conversation}/messages`, data, cb);
        });
    }
};
