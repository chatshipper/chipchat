                ...
                async (response, retryWithMergedOptions) => {
                    if (response.statusCode === 401 && instance.auth.refreshToken
                        ...
                        if (instance.getTokens && typeof instance.getTokens === 'function') {
                            ...
                            if (storeTokens && storeTokens.token !== instance.auth.token) {
                                options.headers.Authorization = `Bearer ${storeTokens.token}`;
                                debug('retry with tokens from store', storeTokens.token, instance.auth.token);
                        ...
                        debug(`using email: ${instance.auth.email} and refresh token ...${instance.auth.refreshToken.slice(-10)}`);
