...
const extendGot = (instance, opts) => {
    ...
    fetch.defaults.options = fetch.mergeOptions(fetch.defaults.options, optionsToAdd);
        ...
        hooks: {
            ...
            afterResponse: [
                (response, retryWithMergedOptions) => {
                    const context = fetch.defaults.options.context;
                    if (response.statusCode === 401 && context.refreshToken
                        ...
                        return fetch(instance._getOptions('POST', 'auth/token', payload)).then((tokens) => {
                            ...
                            fetch.defaults.options = fetch
                                .mergeOptions(fetch.defaults.options, updatedOptions);
                            // Make a new retry
...
const RestAPI = (API) => class extends API {
    ...
    _initApi() {
        ...
        extendGot(this, {
            email: this.auth && this.auth.email,
            token: this.token,
            refreshToken: this.auth && this.auth.refreshToken
        });
    }
    ...
    _getOptions(method, path, json) {
        ...
        const {
            ...
            limit = (pagination && pagination.limit)
                || (json && json.limit) || this.limit || DEFAULTPAGINATIONLIMIT,
    ...
    */
    _request(method, path, json, cb) {
        ...
        if (json && json.pagination && json.pagination.iterate) return fetch.paginate.each(options);
        const func = json && json.pagination ? fetch.paginate.all : fetch;
        const req = func(options).then((response) => {
