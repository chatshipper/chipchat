...
const RestAPI = API => class extends API {
    ...
    */
    _request(method, path, json, cb) {
        ...
        const req = fetch(options.uri, options)
            .then(async (response) => {
                // is the token expired?
                if (response.statusCode === 401 && this.auth.refreshToken && response.body.error.message === 'jwt expired') {
                    ...
                    return fetch(options.uri, retryOptions);
                }
                // pass tru
                return response;
            })
            .then((response) => {
                ...
                if (body && body.error) return Promise.reject(response);
    ...
    _refreshToken() {
        debug('refreshToken', this.auth.email, this.auth.refreshToken);
        const payload = {
